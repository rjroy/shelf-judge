---
title: "Shelf Judge Color System (archived)"
date: 2026-04-11
status: archived
tags: [design, visual, web-ui, color, css, historical]
modules: [web-ui]
related:
...
Archived on 2026-09-13 because the visual design is being redone. The token
names, palette values, dark-mode treatment, and color-specific implementation
rules below are historical and non-authoritative. They must not constrain the
replacement visual system. The separate color-system principles reference is
portable guidance, not an instruction to retain this palette or token set.

This document defines the canonical color token system for `packages/web/app/globals.css`. It is the source of truth for what the `:root` color block contains and why. For background on color-token design in general, see [color-system-principles.md](../color-system-principles.md).

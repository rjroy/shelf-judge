---
title: "Commission: Fix: BGG batch size limit is 20, not 250"
date: 2026-04-06
status: completed
tags: [commission]
worker: Dalton
workerDisplayTitle: "Guild Artificer"
prompt: "The BGG API returns HTTP 400 \"Cannot load more than 20 items\" when batching thing requests. The current `MAX_BATCH_SIZE` in `packages/daemon/src/services/bgg-client.ts` is 250. Change it to 20.\n\nAlso update the BGG research doc at `.lore/research/bgg-api.md` section 6 (Rate Limits) to correct the batch size guidance: the actual limit is 20 IDs per thing request, not 250-500.\n\nAfter fixing, run `bun test` to make sure nothing breaks (some tests may assert on batch behavior with >20 IDs and need adjustment)."
dependencies: []
linked_artifacts:
  - packages/daemon/src/services/bgg-client.ts

  - packages/daemon/tests/services/bgg-client.test.ts
  - .lore/research/bgg-api.md
  - packages/daemon/tests/services/bgg-client.test.ts
  - .lore/research/bgg-api.md
activity_timeline:
  - timestamp: 2026-04-06T03:11:29.698Z
    event: created
    reason: "Commission created"
  - timestamp: 2026-04-06T03:11:29.700Z
    event: status_dispatched
    reason: "Dispatched to worker"
    from: "pending"
    to: "dispatched"
  - timestamp: 2026-04-06T03:11:29.722Z
    event: status_in_progress
    reason: "Execution started"
    from: "dispatched"
    to: "in_progress"
  - timestamp: 2026-04-06T03:11:50.721Z
    event: progress_report
    reason: "Read all three files. Applying the fix: MAX_BATCH_SIZE 250→20 in bgg-client.ts, updating test assertions, and correcting the research doc."
  - timestamp: 2026-04-06T03:12:51.265Z
    event: result_submitted
    reason: "Fixed BGG API batch size limit. Three files changed:\n\n1. `packages/daemon/src/services/bgg-client.ts` — `MAX_BATCH_SIZE` changed from 250 to 20\n2. `packages/daemon/tests/services/bgg-client.test.ts` — batch test updated to use 50 IDs (3 batches of 20+20+10) instead of 300 IDs (2 batches of 250+50)\n3. `.lore/research/bgg-api.md` — Section 6 corrected: the thing endpoint returns HTTP 400 for >20 IDs, not 250-500 as previously documented\n\nAll 222 tests pass."
  - timestamp: 2026-04-06T03:12:55.060Z
    event: status_completed
    reason: "Execution completed"
    from: "in_progress"
    to: "completed"
current_progress: "Read all three files. Applying the fix: MAX_BATCH_SIZE 250→20 in bgg-client.ts, updating test assertions, and correcting the research doc."
projectName: shelf-judge
---
Fixed BGG API batch size limit. Three files changed:

1. `packages/daemon/src/services/bgg-client.ts` — `MAX_BATCH_SIZE` changed from 250 to 20
2. `packages/daemon/tests/services/bgg-client.test.ts` — batch test updated to use 50 IDs (3 batches of 20+20+10) instead of 300 IDs (2 batches of 250+50)
3. `.lore/research/bgg-api.md` — Section 6 corrected: the thing endpoint returns HTTP 400 for >20 IDs, not 250-500 as previously documented

All 222 tests pass.

---
title: "Commission: Mockup: Previously Owned State UI"
date: 2026-04-12
status: completed
tags: [commission]
worker: Sienna
workerDisplayTitle: "Guild Illuminator"
prompt: "Create UI mockups for the Previously Owned State feature based on the spec at `.lore/specs/previously-owned.md`.\n\n**Context:** Shelf Judge is a board game collection curation tool. The web UI is a Next.js app at `packages/web/`. Read the existing pages and components to understand the current visual language before designing.\n\n**Key files to read first:**\n- `.lore/specs/previously-owned.md` (the spec you're mocking up)\n- `packages/web/app/collection/page.tsx` (current collection page)\n- `packages/web/app/game/[id]/page.tsx` (current game detail page)\n- `packages/web/app/globals.css` (current styles, colors, dark mode)\n- `packages/web/components/` (existing component patterns)\n\n**What to mock up:**\n\n1. **Collection page with ownership filter** — Show the collection table with a \"Show Previously Owned\" toggle in the filter bar alongside existing filters. Show what the table looks like with the toggle on: previously-owned games should be visually distinct (muted style, badge, or subtle indicator).\n\n2. **Game detail page for a previously-owned game** — Show ratings, fitness score, and breakdown as normal, but with annotation that niche/redundancy are excluded. Include the \"Mark as Owned\" action to reverse the status.\n\n3. **Game detail page status toggle** — Show the \"Mark as Previously Owned\" action on an owned game's detail page, alongside the existing \"Remove from Collection\" (permanent delete). Make it clear these are different actions with different consequences.\n\n**Design direction:**\n- Match the existing app's visual language (read the CSS and components first)\n- Previously-owned games should be distinguishable at a glance but not jarring\n- The status toggle should feel safe and reversible, unlike the destructive delete action\n- Support both light and dark mode if the app has dark mode"
dependencies: []
linked_artifacts:
  - .lore/mockups/mockup-previously-owned-collection.html

  - .lore/mockups/mockup-previously-owned-game-detail.html
  - .lore/mockups/mockup-previously-owned-game-detail.html
activity_timeline:
  - timestamp: 2026-04-12T21:15:55.311Z
    event: created
    reason: "Commission created"
  - timestamp: 2026-04-12T21:15:55.313Z
    event: status_dispatched
    reason: "Dispatched to worker"
    from: "pending"
    to: "dispatched"
  - timestamp: 2026-04-12T21:15:55.338Z
    event: status_in_progress
    reason: "Execution started"
    from: "dispatched"
    to: "in_progress"
  - timestamp: 2026-04-12T21:23:26.076Z
    event: result_submitted
    reason: "Delivered two HTML mockup files covering all three requested states for the Previously Owned feature.\n\n**Files:**\n- `.lore/mockups/mockup-previously-owned-collection.html` — Collection page with ownership toggle (State A: default owned-only; State B: mixed view with toggle active)\n- `.lore/mockups/mockup-previously-owned-game-detail.html` — Game detail both ownership states (State A: previously-owned game with annotation and \"Mark as Owned\" action; State B: owned game with \"Mark as Previously Owned\" and \"Remove from Collection\" clearly distinguished)\n\n**Key design decisions:**\n\nPreviously-owned row treatment uses warm-neutral archiving palette (#f0ede6 background, muted thumbnail, text-secondary name, PREV OWNED badge in warm-gray) — no new semantic color introduced. The badge follows the same visual weight as niche tags but in a neutral family, signaling \"different\" without signaling \"bad.\"\n\nThe \"Show Previously Owned\" toggle sits in the filter bar as a peer of the Filters and Sort controls, using the existing filter-toggle-btn pattern. An info notice strip explains the data context when the toggle is active (niche/redundancy reflects owned-only set per REQ-PREV-19).\n\nStatus action hierarchy: \"Mark as Previously Owned\" is btn-secondary (outlined, reversible-feeling). \"Remove from Collection\" lives in a separate Danger Zone panel with warning text — different container, different button, physically separated. For a previously-owned game, \"Mark as Owned\" is btn-success (green) because restoration is the recommended path. The asymmetry is intentional.\n\nExcluded breakdown rows for niche/redundancy remain visible in the table with an inline \"Excluded\" label, making the exclusion legible rather than invisible.\n\nBoth files include complete dark mode coverage via media query."
  - timestamp: 2026-04-12T21:23:35.171Z
    event: status_completed
    reason: "Execution completed"
    from: "in_progress"
    to: "completed"
current_progress: ""
projectName: shelf-judge
---
Delivered two HTML mockup files covering all three requested states for the Previously Owned feature.

**Files:**
- `.lore/mockups/mockup-previously-owned-collection.html` — Collection page with ownership toggle (State A: default owned-only; State B: mixed view with toggle active)
- `.lore/mockups/mockup-previously-owned-game-detail.html` — Game detail both ownership states (State A: previously-owned game with annotation and "Mark as Owned" action; State B: owned game with "Mark as Previously Owned" and "Remove from Collection" clearly distinguished)

**Key design decisions:**

Previously-owned row treatment uses warm-neutral archiving palette (#f0ede6 background, muted thumbnail, text-secondary name, PREV OWNED badge in warm-gray) — no new semantic color introduced. The badge follows the same visual weight as niche tags but in a neutral family, signaling "different" without signaling "bad."

The "Show Previously Owned" toggle sits in the filter bar as a peer of the Filters and Sort controls, using the existing filter-toggle-btn pattern. An info notice strip explains the data context when the toggle is active (niche/redundancy reflects owned-only set per REQ-PREV-19).

Status action hierarchy: "Mark as Previously Owned" is btn-secondary (outlined, reversible-feeling). "Remove from Collection" lives in a separate Danger Zone panel with warning text — different container, different button, physically separated. For a previously-owned game, "Mark as Owned" is btn-success (green) because restoration is the recommended path. The asymmetry is intentional.

Excluded breakdown rows for niche/redundancy remain visible in the table with an inline "Excluded" label, making the exclusion legible rather than invisible.

Both files include complete dark mode coverage via media query.

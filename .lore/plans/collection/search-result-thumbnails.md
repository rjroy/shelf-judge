...
related:
  - .lore/issues/collection/search-result-thumbnails.md
...
1. Update the local `BggSearchResult` interface (line 11) to include `thumbnailUrl: string | null`. This maintains the existing duplication with the shared type, which is tracked separately under `.lore/issues/infra/duplicated-web-daemon-types.md`.

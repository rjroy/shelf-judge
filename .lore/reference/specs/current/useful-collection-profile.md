...
The current disposable Profile contract is version 11 and its algorithm is version 13. Ranked attention also has separately versioned candidate state. The durable collection is schema version 8. No Profile cache migration is required: an older contract/algorithm version or a Profile whose serialized entity policy differs from current configuration is discarded and recomputed. Candidate artifact compatibility and recovery follow the ranked-attention authority. A collection migration is required when durable intentions, resolutions, BGG metadata completeness, dated BGG play sessions, attention dispositions, or another collection source field changes schema.

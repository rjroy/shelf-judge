...
`.lore/issues/tournament/deferred-tournament-ranking.md` is still `status: open` despite the tournament feature being fully implemented, spec'd, planned, built, reviewed, and integration-verified across 17 commissions. Needs updating to `resolved`.
...
- **Search result thumbnails**: `BggSearchResult` only returns `bggId/name/yearPublished`. Adding thumbnails requires daemon API changes to include the thumbnail URL from BGG search response. Now tracked as `.lore/issues/collection/search-result-thumbnails.md`.

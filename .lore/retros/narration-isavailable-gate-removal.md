...
date: 2026-04-11
status: complete
tags: [claude-agent-sdk, auth, false-negative, plan-inherited-bug, refactor]
modules: [daemon-narration, web-profile, cli-profile]
related: [.lore/plans/llm-narrative.md, .lore/commissions/commission-Thorne-20260411-174549.md]
---
...
- **The plan prescribed the bug.** `.lore/plans/llm-narrative.md:224` explicitly specified `isAvailable()` checks `ANTHROPIC_API_KEY`. The implementer followed the plan faithfully. Nobody checked whether the gate's premise matched how the Agent SDK actually authenticates.
...
- Plan that prescribed the gate: `.lore/plans/llm-narrative.md` (see Phase 2, section on `isAvailable()`)

...
- BGG thing and collection calls are separate. `bgg-xml-parser.ts` currently accepts lower-bound suggested-player buckets, does not average distinct tied winners, and collapses absent, empty, and unusable poll evidence. `game-service.ts` can retain an old play count while replacing the broad `bggData.fetchedAt`, so that timestamp cannot serve as play-count provenance.
...
- Persist independent usage evidence for play count, published duration, suggested-player poll state, and min/max player range. Each field uses a valid, missing, or invalid evidence variant; invalid evidence preserves a JSON-safe raw envelope without entering calculations. Stable source identifiers are `manual`, `bgg-collection`, `bgg-thing`, `bgg-suggested-player-poll`, `bgg-player-range`, `current-fitness`, and `legacy-unknown`; each observation carries `observedAt: string | null`.
- Retain exact suggested-player buckets and explicit `absent | empty | unusable | usable` poll state. Resolve modeled player count from this evidence on read so ties, ignored lower bounds, range fallback, source, and observation time remain explainable. Do not overwrite `bestPlayers` or alter Player Count Fit axis semantics for this feature.
...
2. Return exact suggested-player poll buckets and distinguish absent poll XML, present-but-empty results, unusable buckets, and usable evidence in the BGG result.
3. Accept only exact positive safe-integer bucket labels. Ignore `N+`, nonnumeric, zero, negative, and unsafe buckets. Require positive Best votes, choose the greatest vote count, and average all distinct tied winning counts exactly.
...
- Parser/result tests cover one winner, distinct ties and exact average, duplicate buckets, `N+`, zero Best votes, absent, empty, unusable, valid range data, invalid range data, and no evidence.
...
1. Define persisted amount, acquisition, benchmark, observation, poll-state, and usage-evidence unions. Valid evidence has a validated payload; missing evidence has no payload; invalid evidence uses `{ presence: "missing" }` or `{ presence: "present", value: JsonValue }` so it is serializable and distinguishable. Keep invalid values inaccessible to calculation helpers.
...
4. Add a pure `2 -> 3` migration that preserves every valid existing game field, adds unknown acquisition and no benchmark, and maps existing play count, duration, range, and suggested-player values to valid, missing, or invalid `legacy-unknown` evidence without timestamps. Preserve malformed raw values rather than blessing or discarding them.
5. Define migration behavior for zero/negative/unsafe duration, play count, player bounds, reversed ranges, malformed poll buckets, and malformed `bestPlayers`. Replace a malformed compatibility field with its strict-schema-safe value, normally `null` or `[]`, while retaining the exact original JSON in the corresponding invalid evidence envelope. Play-count and duration evidence preserve malformed `numPlays` and `playingTime`; one player-count evidence envelope preserves malformed `bestPlayers`, suggested-player buckets, and range fields together.
...
7. Update fresh collection creation, game add/import/refresh, prediction preview, and every persisted or temporary `Game` constructor to version-3 types. Persist Step 2 observations exactly and retain old evidence unchanged when a response omits that field.
...
- Valid version 2 fields remain byte-equivalent outside the new fields. Valid old evidence becomes `legacy-unknown` with null observation time. Malformed compatibility fields become strict-safe while their exact original JSON remains in the named invalid evidence envelope.
- Migration tests cover zero/negative duration, invalid play counts, zero/negative/unsafe/reversed player ranges, malformed polls, unsafe `bestPlayers`, and missing properties.
...
3. Resolve modeled player count through Step 3 evidence and attach source, observation time, tie/fallback details, all inputs, and only component-relevant stable reasons.
...
| 10. Player-count evidence               | `packages/daemon/tests/services/bgg-xml-parser.test.ts` and `game-service-bgg.test.ts`                |
...
- **Suggested-player behavior is shared data:** keep utilization resolution separate from existing `bestPlayers` behavior unless all other consumers and tests are intentionally migrated.

...
date: 2026-04-11
status: executed
tags: [plan, web-ui, bgg, search]
modules: [shared, daemon, web]
related:
  - .lore/work/issues/search-result-thumbnails.md
...
The BGG `/search` endpoint returns only `id`, `name`, and `yearpublished` per result. No thumbnail or image data. This is confirmed by both the API research (`.lore/archive/research/bgg-api.md`, section 4) and the test fixture (`packages/daemon/tests/fixtures/search-wingspan.xml`).
...
1. Update the local `BggSearchResult` interface (line 11) to include `thumbnailUrl: string | null`. This maintains the existing duplication with the shared type, which is tracked separately under `.lore/work/issues/duplicated-web-daemon-types.md`.

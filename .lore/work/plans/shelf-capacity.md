...
related:
  ...
  - .lore/work/mockups/mockup-shelf-game-dimensions.html
  - .lore/work/mockups/mockup-shelf-configuration.html
  - .lore/work/mockups/mockup-shelf-capacity-indicator.html
  - .lore/work/mockups/mockup-shelf-capacity-detail.html
  - .lore/work/mockups/mockup-shelf-capacity-detail-empty.html
---
...
Implement the shelf capacity feature specified in `.lore/reference/specs/features/shelf-capacity.md` (REQ-SHELF-1 through REQ-SHELF-36). Three connected layers: box dimensions on games, shelf configuration as a separate storage entity, and a bin-packing-driven capacity computation that tells users what fits, what doesn't, and where to cull.

The bin-packing algorithm (`.lore/reference/designs/similarity-weighted-bin-packing.md`) is implemented as a standalone module with no Shelf Judge domain knowledge. Layer 3 builds an adapter between the project's data model and the algorithm's generic item/bin interface.
...
The mockup at `.lore/work/mockups/mockup-shelf-game-dimensions.html` defines the layout. Follow the existing metadata display pattern on the game detail page.
...
**6b: Shelf configuration page.** Create `packages/web/app/shelves/page.tsx` as a client component (needs interactivity for add/edit/remove/reorder). The mockup at `.lore/work/mockups/mockup-shelf-configuration.html` defines the layout:
...
This is the algorithm defined in `.lore/reference/designs/similarity-weighted-bin-packing.md`, implemented as a standalone module. It has NO imports from `@shelf-judge/shared` and NO knowledge of games, shelves, or fitness scores. It operates on generic items and bins.
...
The mockup at `.lore/work/mockups/mockup-shelf-capacity-indicator.html` defines the visual design.
...
1. **Shelf assignments**: each shelf as a card with assigned games, utilization bar, and grade badge. The mockup at `.lore/work/mockups/mockup-shelf-capacity-detail.html` defines the layout. Grade badges use the `--grade-*` CSS tokens from the mockup.
...
Empty states: when no config exists or no dimensioned games, show appropriate messages per `.lore/work/mockups/mockup-shelf-capacity-detail-empty.html`.
...
1. Reads the spec (`.lore/reference/specs/features/shelf-capacity.md`) and this plan
...
Review C13 against the design doc (`.lore/reference/designs/similarity-weighted-bin-packing.md`). Verify standalone constraint (no `@shelf-judge/shared` imports). Verify rotation edge cases, merge strategy correctness, grading formula.

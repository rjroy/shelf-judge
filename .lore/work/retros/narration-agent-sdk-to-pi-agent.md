...
- Stale references worth a follow-up sweep (not in scope for this change):
  - `.lore/work/plans/llm-narrative.md` — marked `status: executed`; still describes the Agent SDK integration
  - `.lore/archive/research/claude-agent-sdk.md` — reference doc; will need an equivalent for pi if future work touches narration

...
Shelf Judge must let the owner ask their own questions about their collection and receive inspectable answers grounded in evidence Shelf Judge is authorized to disclose. The owner defines the analytical job in natural language and judges whether an answer is useful. Shelf Judge does not limit chat to a menu of prewritten questions.
...
### Owner delivery decision, 2026-09-07

The first usable milestone is an explicitly initiated web conversation: an owner
can ask and follow up, see validated progress and citations, stop, retry, or
start a new ephemeral conversation. No scheduler, background trigger, proactive
analysis, or new agent subsystem is in scope. The owner alone judges usefulness.
Corpus quotas, blinded or independent review, authorship/provenance packets,
baseline comparison, and usefulness scoring are not release gates. Deterministic
evidence authorization, citation and schema validation, privacy, capability
isolation, read-only authority, cancellation, and regression fixtures remain
required. CLI parity and final technical validation remain later work and do not
block the first web milestone.
...
4. **REQ-ANALYST-4:** Every substantive collection claim must cite at least one validated turn-scoped evidence item; unknown, stale, unauthorized, fabricated, cross-conversation, or unreferenced citations must prevent a successful final response. Citation validation establishes provenance and structure only, not semantic relevance, entailment, usefulness, or truth.
...
- Automated semantic proof, reviewer scoring, corpus quotas, or usefulness evaluation as a release gate
...
4. For every substantive sentence in answered and partial fixtures, resolve adjacent citation IDs to the turn registry and canonical source. Inject missing, invented, stale, wrong-game, wrong-version, cross-turn, cross-conversation, unauthorized, and unreferenced citations and verify runtime validation emits no successful final response. Keep structurally valid but irrelevant, misleading, and non-entailing cases as deterministic regression fixtures without claiming that runtime validation proves semantics. Enumerate every field in synthetic provider payloads against the exact authorization-manifest version and fail on any unnamed field.
...
24. Retired by the 2026-09-07 owner decision: no versioned corpus quota, blinded review, authorship artifact, reviewer score, baseline comparison, or usefulness threshold gates delivery. Preserve deterministic fixtures for closed-manifest payloads, citations, schema rejection, privacy, capability isolation, cancellation, abstention, and read-only authority; they do not prove usefulness or truth.
...
26. Keep documentation precise enough for deterministic technical validation of open-ended questions with bounded answers, disclosure, testimony, citations, ephemerality, cancellation/retry, cap policy, and read-only authority. A fresh subjective reviewer is optional feedback, not a mandatory gate.

...
> authority. Its console-call counts are stale, and remaining `console.warn`
> usage may be an implementation bug rather than supersession. Preserve the
> logger-factory requirements while reconciling them against the daemon code.
...
This issue was surfaced during the BGG import double-request bug investigation (`.lore/work/retros/bgg-import-double-request.md`). The retro explicitly noted: "Logging at integration boundaries is not optional. 'Add logging later' means 'debug blind until someone complains.'" The logging itself was added in that fix; this spec addresses the remaining cleanup of centralizing the format.

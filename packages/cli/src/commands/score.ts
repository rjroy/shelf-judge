...
export async function scoreList(client: DaemonClient, _args: string[], opts: OutputOptions): Promise<string> {
...
export async function scoreGet(client: DaemonClient, args: string[], opts: OutputOptions): Promise<string> {
  const id = args[0];
  if (!id) {
    throw new Error("Usage: shelf-judge score get <id>");
  }

  const { ok, data } = await client.get<ScoreGetResponse>(`/api/games/${encodeURIComponent(id)}/score`);
  ...
  lines.push(`Fitness: ${formatScore(result.score)} (${result.ratedAxisCount}/${result.totalAxisCount} axes rated)`);
  lines.push("");

...
export interface UsefulProfileInput {
  collection: Collection;
  fitnessResults: ReadonlyMap<string, FitnessResult>;
  computedAt: string;
}

const PROFILE_ENTITY_CLASSES: ProfileEntityClass[] = ["mechanic", "designer", "artist"];
...
function profileFitnessEvidence(game: Game, fitness: FitnessResult): ProfileGameFitnessEvidence {
  return {
    gameId: game.id,
    gameName: game.name,
...
function exactFitnessMean(games: ProfileGameFitnessEvidence[]): ExactRational {
  return games
    .reduce(
...
function entityOrderings(entities: ProfileEntityEvidence[]) {
...
function computeEntityClass(
  ...
  const exclusions: ProfileClassExclusion[] = [];
  const comparatorGames: ProfileGameFitnessEvidence[] = [];
  ...
  const observations = new Map<
    number,
    { name: string; observedAt: string; gameId: string; evidence: ProfileGameFitnessEvidence }[]
  >();
  for (const game of ownedGames) {
  ...
  const entities = [...observations].map(([entityId, values]): ProfileEntityEvidence => {
...
function attentionPlayEvidence(game: Game): PlayIntentionAttentionItem["currentPlayEvidence"] {
...
export function computeUsefulProfile(input: UsefulProfileInput): FutureUsefulCollectionProfile {
  ...
  return {
    ...
    identity: {
      ...
      ) as Record<ProfileEntityClass, ProfileEntityClassResult>,
      axisDistributions: computeAxisDistributions(
